...
file = "outputs/send_commands_" + date + ".txt"
...
async def connect_and_execute(router):
    ...
    async with AsyncJunosDriver(**device) as conn:
        ...
        async with aiofiles.open(file, "a") as myfile:
            await myfile.write(f"### Enviando o comando para o roteador { router } ####\n ")   
            await myfile.write(response.result)
...
async def main():
    ...
    try:
        ...
        async with aiofiles.open(file, "a") as myfile:
            await myfile.write(f"Não foi possível acessar o roteador {router}")
